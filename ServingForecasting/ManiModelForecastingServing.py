...
# MAGIC
# MAGIC There are a lot of models such a Prophet that requires you to provide the forecasting_Horizon, so let's add a horizon here. 
# MAGIC
...
def fit_final_model_udf(df_pandas: pd.DataFrame) -> pd.DataFrame:
    ...
    NOTE: 
    In a case you are using a very simple model - Linear Model, it does not contain parameters,
